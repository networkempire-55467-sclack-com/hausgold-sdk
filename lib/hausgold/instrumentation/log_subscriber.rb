...
module Hausgold
  module Instrumentation
    ...
    class LogSubscriber < ActiveSupport::LogSubscriber
      # Return the HAUSGOLD SDK configured logger when logging is enabled.
      # Otherwise +nil+ is returned and the subscriber is never started.
      #
      # @return [Logger, nil] the logger to use
      def logger
        return unless Hausgold.configuration.request_logging

        Hausgold.configuration.logger
      end
      ...
      def req_origin(env)
        ...
        "#{Hausgold.env}/#{req[:client]}##{action}"
      end
