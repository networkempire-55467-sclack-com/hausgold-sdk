...
RSpec.describe Hausgold::Request::DefaultHeaders do
  let(:app) { proc {} }
  let(:instance) do
  ...
  describe '#user_agent' do
    it 'identifies the client' do
      expect(instance.user_agent).to start_with('HausgoldSDK')
    end

    it 'sets the client version' do
      expect(instance.user_agent).to include(Hausgold::VERSION)
    end

    it 'adds the configured application name' do
      Hausgold.configuration.app_name = 'test_app'
      expect(instance.user_agent).to end_with('test_app')
    end
  end
end
